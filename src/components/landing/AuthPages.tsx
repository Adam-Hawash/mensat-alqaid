...
export function LoginView() {
  ...
  var password = passState1[0]
  var setPassword = passState1[1]
  ...
  var handleLogin = async function () {
    ...
    try {
      ...
      // Student login
      var res = await fetch('/api/students?phone=' + encodeURIComponent(phone.trim()) + '&password=' + encodeURIComponent(password.trim()) + '&deviceId=' + encodeURIComponent(getDeviceId()) + '&deviceIds=' + encodeURIComponent(JSON.stringify(getDeviceCandidates())) + '&deviceType=' + encodeURIComponent(getDeviceType()))
    ...
    <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center px-4 py-8">
      <motion.div className="w-full max-w-lg" initial="hidden" animate="visible" variants={fadeInUp} transition={{ duration: 0.5, ease: 'easeOut' }}>
        ...
        <div className="text-center mb-6">
          <div className="inline-flex items-center justify-center h-14 w-14 rounded-2xl bg-gradient-to-br from-yellow-400 to-orange-500 mb-3"><Users className="h-7 w-7 text-white" /></div>
